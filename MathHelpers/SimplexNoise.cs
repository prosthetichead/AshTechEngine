...
/// </summary>
namespace AshTechEngine.MathHelpers
{
    public static class SimplexNoise
    {
        ...
        public static float[,,] Calc3D(int width, int height, int length, float scaleWidth, float scaleHeight, float scaleLength)
        {
            var values = new float[width, height, length];
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++)
                    for (var k = 0; k < length; k++)
                        values[i, j, k] = Generate(i * scaleWidth, j * scaleHeight, k * scaleLength) * 128 + 128;
            return values;
        }
        ...
        public static float CalcPixel3D(int x, int y, int z, float scaleX, float scaleY, float scaleZ)
        {
            return Generate(x * scaleX, y * scaleY, z * scaleZ) * 128 + 128;
        }
